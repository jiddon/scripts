...
def get_lumi_daily():
    df = dd.read_csv('./lumi_files/daytable.csv')
    df = df.rename(columns=lambda x: x.strip())
    return df

def get_rod_data(name, meas):
    df = dd.read_csv('./dcs_csv/'+name+'_'+meas+'.csv', names=["Day", meas])
    return df

def read_csv_to_df(path):
    df = dd.read_csv(path, names=["Day", path], dtype={0:int,1:float})
    return df
...
def get_corr_dict(df, columns, obs, corr_cut, verbose):
    """
...
def get_dfs(rod, obs):
    ...
    df_merged = reduce(lambda  left,right: dd.merge(left,right,on=['Day'],
...
class ci_functions():
    ...
    def find_corr(self, rod, *obs, plot=True, corr_cut=0.7, verbose=False, from_paths=None):
        ...
        # correlation matrix
        corr_dict = get_corr_dict(df, columns, obs, corr_cut, verbose)
        if verbose:
            print(corr_dict)
    ...
    def find_dcs_corr(self, filename, plot=True, corr_cut=0.7, verbose=False, matrix=False, from_path=None):
        ...
        file_path = filename
        if exists(file_path):
            paths_ = sf.split_files(file_path)
            paths = []
            for p in paths_:
                paths.append(sf.daily_avg(p))
            rods_obs = {}
            print(paths)
            for p in paths:
